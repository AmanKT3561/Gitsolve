// GitSolve AI — naukri detector (runs in world:'MAIN', fully self-contained, NO imports)
(function () {
  if (window.__gitsolve_naukri) return;        // double-injection guard
  window.__gitsolve_naukri = true;
  ...
  function gsHookFetch(handler) {
    if (window.__gitsolve_fetch_hooked_naukri) return;
    window.__gitsolve_fetch_hooked_naukri = true;
  ...
  // Code360 (naukri.com/code360, codingninjas.com): fetch intercept + DOM fallback.
  gsHookFetch(function (url, data) {
    if (url.indexOf('submit') === -1 && url.indexOf('run') === -1) return;
    var v = data && (data.verdict || (data.data && data.data.verdict) || data.status || (data.data && data.data.status));
    if (!v || String(v).toLowerCase().indexOf('accept') === -1) return;
    gsTryEmit(data, 'fetch');
  });

  function gsTryEmit(data, source) {
    var slug = (location.pathname.match(/problem(?:s)?\/([^\/?#]+)/) || location.pathname.match(/\/([^\/?#]+)_\d+/) || [])[1] || 'unknown';
    var key = (data && (data.id || data.submissionId)) || (slug + ':' + source + ':' + Date.now());
    if (window._gsSeen.has('nk' + key)) return;
    window._gsSeen.add('nk' + key);
    gsPost({
      platform: 'naukri',
      externalSubmissionId: String(key),
      problemSlug: slug,
      problemTitle: gsText('h1, .problem-name, .heading') || slug,
      problemUrl: location.href.split('?')[0],
      difficulty: gsDifficulty(gsText('[class*="difficulty"], .difficulty')),
  ...
  // DOM fallback: a "Correct Answer" / "Accepted" banner appears.
  gsObserve(function () {
    var banner = document.querySelector('[class*="accepted"], [class*="correct"], .submission-status');
    if (!banner) return;
    var txt = (banner.textContent || '').toLowerCase();
    if (txt.indexOf('accept') === -1 && txt.indexOf('correct') === -1) return;
    gsTryEmit({}, 'dom');
  });
