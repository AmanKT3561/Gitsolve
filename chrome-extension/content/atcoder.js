// GitSolve AI — atcoder detector (runs in world:'MAIN', fully self-contained, NO imports)
(function () {
  if (window.__gitsolve_atcoder) return;        // double-injection guard
  window.__gitsolve_atcoder = true;
  ...
  function gsHookFetch(handler) {
    if (window.__gitsolve_fetch_hooked_atcoder) return;
    window.__gitsolve_fetch_hooked_atcoder = true;
  ...
  // AtCoder: a verdict of AC shows as <span class="label-success">AC</span>.
  // On the submission detail page the source is in pre#submission-code.
  gsObserve(function () {
    var label = null;
    var labels = document.querySelectorAll('span.label-success, td.text-center .label-success');
    for (var i = 0; i < labels.length; i++) {
      if ((labels[i].textContent || '').trim() === 'AC') { label = labels[i]; break; }
    }
    if (!label) return;

    var idMatch = location.pathname.match(/submissions\/(\d+)/);
    if (!idMatch) return; // only act on a concrete submission page
    var subId = idMatch[1];
    if (window._gsSeen.has('ac' + subId)) return;
    window._gsSeen.add('ac' + subId);

    var taskMatch = location.pathname.match(/contests\/([^\/]+)\/submissions/);
    var contest = taskMatch ? taskMatch[1] : 'atcoder';
    var taskCell = document.querySelector('a[href*="/tasks/"]');
    var task = '';
    if (taskCell) { var t = taskCell.getAttribute('href').match(/tasks\/([^\/?#]+)/); task = t ? t[1] : ''; }
    var slug = task || (contest + '-' + subId);
    var title = (taskCell && (taskCell.textContent || '').trim()) || slug;

    var langCell = document.querySelector('td a[href*="/submissions?f.Language"], table td:nth-child(4)');
    gsPost({
      platform: 'atcoder',
      externalSubmissionId: String(subId),
      problemSlug: slug,
      problemTitle: title,
      problemUrl: location.origin + '/contests/' + contest + (task ? '/tasks/' + task : ''),
      difficulty: 'Unknown',
      language: (langCell && (langCell.textContent || '').trim()) || 'unknown',
      code: gsGetCode(),
      topics: [],
    });
  });
